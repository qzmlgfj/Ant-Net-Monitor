...
def create_cpu_alarm(app):
    with app.app_context():
        ...
        db.session.add(AlarmItem(name='cpu_usage', alarm_id=cpu_alarm.id, alarm_value=0.8))
        db.session.add(AlarmItem(name='cpu_iowait', alarm_id=cpu_alarm.id, alarm_value=0.4))
        db.session.add(AlarmItem(name='cpu_steal', alarm_id=cpu_alarm.id, alarm_value=0.1))
        db.session.commit()
...
def create_ram_alarm(app):
    with app.app_context():
        ...
        db.session.add(AlarmItem(name='ram_usage', alarm_id=ram_alarm.id, alarm_value=0.8))
        db.session.commit()
...
def get_all_alarm_items(app):
    with app.app_context():
        try:
            cpu_alarm = Alarm.query.filter_by(name='CPU').first()
            ram_alarm = Alarm.query.filter_by(name='RAM').first()
            return AlarmItem.query.filter(AlarmItem.alarm_id.in_([cpu_alarm.id, ram_alarm.id])).all()
        except AttributeError:
            return None
