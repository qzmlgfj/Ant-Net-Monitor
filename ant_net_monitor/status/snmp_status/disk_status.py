...
class DiskStatus:
    ...
    def get_last(self):
        ...
        return (
            DiskStatusInfo.query.filter(DiskStatusInfo.time_stamp > start)
            .order_by(DiskStatusInfo.time_stamp.desc())
            .first()
        )

    def get_batch(self):
        count = DiskStatusInfo.query.count()
        if count > 100:
            count = 100
        return (
            DiskStatusInfo.query.order_by(DiskStatusInfo.time_stamp.desc())
            .limit(count)
            .all()[::-1]
        )
    ...
    def get_in_one_day(self):
        ...
        return (
            DiskStatusInfo.query.filter(DiskStatusInfo.time_stamp > start)
            .filter(extract("minute", DiskStatusInfo.time_stamp) % 5 == 0)
            .filter(extract("second", DiskStatusInfo.time_stamp) == 0)
            .all()
        )
